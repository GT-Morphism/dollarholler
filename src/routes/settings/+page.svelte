<script lang="ts">
	...
	onMount(() => {
		...
		// Necessary for avoiding 500 error; otherwise need to use if-block in HTML
		// if $settings would have been used directly;
		mySettings = { ...$settings };
	});
</script>
	...
	>
		...
		<form class="grid grid-cols-6 gap-x-5">
			...
			<div class="field col-span-6 flex justify-end">
				<Button label="Save" iconLeft={CheckIcon} on:click={() => {}} />
			</div>
		</form>
